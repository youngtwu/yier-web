...
//        logger.info("[1]发送请求参数信息:{}",JsonUtils.toJsonString(orderDelayQueue));
        ...
        rabbitTemplate.convertAndSend(applicationConfig.getRabbitConfig().getOrderDelayExchange(), RabbitMQConfig.ORDER_DELAY_ROUTING_KEY, sendMessage, new MessagePostProcessor() {
            @Override
...
//    @RabbitListener(queues = {applicationConfig.getRabbitConfig().getOrderQueueName()})
//    @RabbitListener(queues = {"${common.application.config.rabbitConfig.orderQueueName}"})
    public void orderDelayQueue(Message message, Channel channel) {
        ...
        try {
            ...
            switch (orderDelayQueue.getCaseInfo()){
                case ConstantAll.CASE_INFO_ORDER_MEDICAL_PAYMENT_TIME:
                    ...
                    if(resultMedicalOrderTimeOut!=null){
                        ...
                        logger.info("[门诊单待支付]患者过期[已经完成逻辑处理]");
                        oprationCase = "[门诊单待支付][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[门诊单待支付]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_MEDICAL_PRES_CHECK_A:
                    ...
                    if(resultCheckA!=null){
                        ...
                        oprationCase = "[药师审方环节A][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师审方环节A]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_MEDICAL_PRES_CHECK_B:
                    ...
                    if(resultCheckB!=null){
                        ...
                        oprationCase = "[药师审方环节B][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师审方环节B]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_MEDICAL_PRES_CHECK_C:
                    ...
                    if(resultCheckC!=null){
                        ...
                        oprationCase = "[药师审方环节C][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师审方环节C]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_MEDICAL_PRES_CHECK_D:
                    ...
                    if(resultCheckD!=null){
                        ...
                        logger.info("[药师审方环节D]药师抢单阶段时间到期[已经完成逻辑处理]");
                        oprationCase = "[药师审方环节D][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师审方环节D]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_MEDICAL_PRES_CHECK_E:
                    ...
                    if(resultCheckE!=null){
                        ...
                        oprationCase = "[药师审方环节E][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师审方环节E]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_ORDER_TAKE_SEND_PAYMENT:
                    logger.info("[处方待支付]患者看到的处方支付过期 延迟处理逻辑");
                    ...
                    if(resultPaymentTimeOutTaskSend!=null){
                        ...
                        logger.info("[处方待支付]患者看到的处方支付过期[已经完成逻辑处理]");
                        oprationCase = "[处方待支付][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[处方待支付]扑空";
                    }
                    break;
                case ConstantAll.CASE_INFO_ORDER_SEND_CHECK_A:
                    ...
                    if(resultPharmacistCheckA!=null){
                        ...
                        oprationCase = "[药师接单A][已经完成逻辑处理]";
                    }
                    else {
                        oprationCase = "[药师接单A]扑空";
                    }
            ...
            redisLogService.insertSelective(new RedisLog(ConstantAll.LOG_RABBIT, orderDelayQueue.getCaseInfo(), orderDelayQueue.getRedisKey(), Utils.formatDateToyyyyMMddHHmmss(new Date()), 0L, ConstantAll.STRING_EMPTY, ConstantAll.LOG_REMARK_DELETE + "[结束]"+oprationCase));
