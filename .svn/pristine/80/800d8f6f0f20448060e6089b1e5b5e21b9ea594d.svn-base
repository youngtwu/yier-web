...
public class AuditInfoService {
    ...
    public MutableTriple<HospitalPo, AuditInfo, List<AuditInfo>> selectHospitalAuditInfo(HospitalRequest params) {
        ...
        if(hospitalPo != null){
            //根据id查询省份，城市，区域名称
            Provinces provinces = this.districtService.selectProvinceById(Integer.valueOf(hospitalPo.getProvinceId()));
            if (provinces != null) {
                hospitalPo.setProvinceName(provinces.getProvince());
            }
            Cities city = this.districtService.selectCityById(hospitalPo.getCityId());
            if (city != null) {
                hospitalPo.setCityName(city.getCity());
            }
            Areas areas = this.districtService.selectAreaById(hospitalPo.getAreaId());
            if (areas != null) {
    ...
    public MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>> selectMessageAuditInfo(MessagesRequest params) {
        AuditInfoRequest requestParams = new AuditInfoRequest();
        ...
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>>();
        mutableTriple.left = messagesPo;
        if(auditInfoList != null && auditInfoList.size() > 0){
            mutableTriple.middle = auditInfoList.get(0);
        }
        mutableTriple.right = auditInfoList;
    ...
    public MutableTriple<NewsPo, AuditInfo, List<AuditInfo>> selectNewsAuditInfo(NewsRequest params) {
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(params.getNewId());
        requestParams.setAuditType(4);
        NewsPo newsPo = this.newsService.getNewsById(params);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<NewsPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<NewsPo, AuditInfo, List<AuditInfo>>();
        mutableTriple.left = newsPo;
        if(auditInfoList != null && auditInfoList.size() > 0){
            mutableTriple.middle = auditInfoList.get(0);
        }
        mutableTriple.right = auditInfoList;
    ...
    public MutableTriple<UserPo, AuditInfo, List<AuditInfo>> selectAuditInfoByUserIdAndClientType(UserRequest params) {
        Assert.isTrue(params.getClientType() != null, "端口类型(用户所属端口类型:1.医生，2.药师，3.患者 4.亿尔APP后台管理 6.医药信息后台管理)不能为空！");
    ...
    @Transactional
    public MutableTriple<HospitalPo, AuditInfo, List<AuditInfo>> auditHospital(Hospital hospital, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(hospital.getId());
        requestParams.setAuditType(1);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<HospitalPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<HospitalPo, AuditInfo, List<AuditInfo>>();
    ...
    @Transactional
    public MutableTriple<DoctorPo, AuditInfo, List<AuditInfo>> auditDoctor(Doctor doctor, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(doctor.getId());
        requestParams.setAuditType(2);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<DoctorPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<DoctorPo, AuditInfo, List<AuditInfo>>();
    ...
    @Transactional
    public MutableTriple<PharmacistPo, AuditInfo, List<AuditInfo>> auditPharmacist(Pharmacist pharmacist, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(pharmacist.getId());
        requestParams.setAuditType(3);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<PharmacistPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<PharmacistPo, AuditInfo, List<AuditInfo>>();
    ...
    @Transactional
    public MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>> auditMessages(Messages messages, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(messages.getId());
        requestParams.setAuditType(5);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<MessagesPo, AuditInfo, List<AuditInfo>>();
    ...
    @Transactional
    public MutableTriple<NewsPo, AuditInfo, List<AuditInfo>> auditNews(News news, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(news.getId());
        requestParams.setAuditType(5);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<NewsPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<NewsPo, AuditInfo, List<AuditInfo>>();
    ...
    @Transactional
    public MutableTriple<UserPo, AuditInfo, List<AuditInfo>> auditMobilePhoneNumber(UserRequest params, Long userId, String content) {
        ...
        AuditInfoRequest requestParams = new AuditInfoRequest();
        requestParams.setTableId(userId);
        requestParams.setAuditType(6);
        List<AuditInfo> auditInfoList = this.auditInfoMapper.selectAuditInfoByTableId(requestParams);
        MutableTriple<UserPo, AuditInfo, List<AuditInfo>> mutableTriple = new MutableTriple<UserPo, AuditInfo, List<AuditInfo>>();
        mutableTriple.left = userPo;
