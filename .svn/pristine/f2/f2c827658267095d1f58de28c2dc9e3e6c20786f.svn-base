...
public class MedicalOrderService {
    ...
    public MedicalOrder createPatientPaymentCancel(MedicalOrder record){
        ...
        Assert.isTrue(StringUtils.equalsIgnoreCase(currentMedicalOrder.getStatus(),ConstantAll.ORDER_STATUS_APPPOINT),"只能取消待支付的订单! (目前状态是："+currentMedicalOrder.getStatus()+")");
        ...
        medicalOrder.setRemarks("患者支付取消");
        ...
        medicalDate.setRemarks("患者支付取消 预约锁定-1");
...
//        Utils.formatDate()
        ...
        else if(currentMedicalOrder.getWorkFrom().getTime() - currentDate.getTime() > ConstantAll.VALUE_TIME_48_HOUR*ConstantAll.VALUE_MICROSECOND_1000){
            MedicalOrder medicalOrder = new MedicalOrder();
            medicalOrder.setId(record.getId());
            medicalOrder.setStatus(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
            medicalOrder.setStatusDetail(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
        ...
        else if(currentMedicalOrder.getWorkFrom().getTime() - currentDate.getTime() > ConstantAll.VALUE_TIME_24_HOUR*ConstantAll.VALUE_MICROSECOND_1000){
            MedicalOrder medicalOrder = new MedicalOrder();
            medicalOrder.setId(record.getId());
            medicalOrder.setStatus(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
            medicalOrder.setStatusDetail(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
        ...
        else if(currentMedicalOrder.getWorkFrom().getTime() - currentDate.getTime() > 0){
            MedicalOrder medicalOrder = new MedicalOrder();
            medicalOrder.setId(record.getId());
            medicalOrder.setStatus(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
            medicalOrder.setStatusDetail(ConstantAll.ORDER_STATUS_PATIENT_CANCEL);//预约确认
    ...
    public MedicalOrder createDoctorCancel(MedicalOrder record){
        ...
        if(currentMedicalOrder.getWorkFrom().getTime() - currentDate.getTime() > ConstantAll.VALUE_TIME_48_HOUR*ConstantAll.VALUE_MICROSECOND_1000){
            MedicalOrder medicalOrder = new MedicalOrder();
            medicalOrder.setId(record.getId());
...
/*        medicalDateRequest.setTypeId(ConstantAll.TYPE_ID_DOCTOR+"");
        ...
        Assert.isTrue(medicalDatePo!=null, Utils.formatDate(record.getPlanTime(),"yyyy-MM-dd")+record.getWorkTime()+" 医生没有就诊安排,无法预约!");
        Assert.isTrue(medicalDatePo.getAppointmentNum() >= 1,Utils.formatDate(record.getPlanTime(),"yyyy-MM-dd")+record.getWorkTime()+"医生该日期的上门就诊已经饱和,无法预约!");
...
//            t.getPatientPo().thinkField();
    ...
    public void taskConsiderOrderPastDue(Date currentDate){
        ...
        considerStatus.add(ConstantAll.ORDER_STATUS_START_SERVICE);//4
        ...
        medicalOrderList.forEach(order->{
            ...
            medicalOrder.setRemarks(Utils.formatDateToyyyyMMddHHmmss(currentDate)+ "定时检查到没有按预约期限完成任务，作为医生预约过期  超时未到，目前工作时间:"+Utils.formatDateToyyyyMMddHHmmss(order.getWorkTo()));
            medicalOrder.setOverType(ConstantAll.ORDER_END_TYPE_DOCTOR_APPPOINT_PAST_DUE);
            medicalOrder.setOverTime(currentDate);
            this.updateByPrimaryKeySelective(medicalOrder);
        });
