...
public class AttentionUserServiceTest extends TestBase {
    ...
    @Test
    public void listAttentionUser() {
        List<AttentionUserPo> list = this.serviceAttentionUserService.listAttentionUserPo(params);
        logger.info("查询到的个数是:{}", list.size());
    }

    @Test
    public void listAttentionUserCount() {
        int size = this.serviceAttentionUserService.listAttentionUserPoCount(params);
        logger.info("查询到的个数是:{}", size);
    }
