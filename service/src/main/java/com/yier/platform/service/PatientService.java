...
public class PatientService {
    ...
    public PatientPo loginByVerifyCode(PatientRequest params, String verifyCode, String osType) {
        ...
        smsService.checkCode(model.getPhoneNumber(), ConstantAll.TYPE_ID_PATIENT, verifyCode);//验证码验证
        ...
        modelPo.setToken(this.jwtTokenService.createToken(""+ConstantAll.TYPE_ID_PATIENT,modelPo.getId().toString(),osType,modelPo.getPhoneNumber()));
        modelPo.setNonce(RandomUtils.randomNumeric(4));
        modelPo.setAppKey(this.signatureService.getAppKeyBy(ConstantAll.TYPE_ID_PATIENT,modelPo.getNonce()));
...
//        RongYunTokenResult ry = this.serviceRongCloudService.getToken(ConstantAll.TYPE_ID_PATIENT, record.getId(), record.getTrueName(), record.getAvatarUrl());
//        if (ry == null) {
