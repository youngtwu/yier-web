...
public class PharmacistService {
    ...
    public List<Pharmacist> getListAllPlat() {
        ...
        if(listAllPlat==null || listAllPlat.size()==0){
            ...
            redisService.setJsonObjectBy(ConstantAll.REDIS_ALL_PLAT_PHARMACIST,listAllPlat,12L,TimeUnit.HOURS);
//            redisService.setJsonObjectBy(ConstantAll.REDIS_ALL_PLAT_PHARMACIST,listAllPlat,30L,TimeUnit.SECONDS);
    ...
    public List<Pharmacist> getListAllCheck() {
        ...
        if(listAllCheck==null || listAllCheck.size()==0){
            ...
            redisService.setJsonObjectBy(ConstantAll.REDIS_ALL_CHECK_PHARMACIST,listAllCheck,12L,TimeUnit.HOURS);
//            redisService.setJsonObjectBy(ConstantAll.REDIS_ALL_CHECK_PHARMACIST,listAllCheck,30L,TimeUnit.SECONDS);
    ...
    public PharmacistPo getPharmacistPoById(PharmacistRequest params) {
        ...
        paramsPr.setTypeId(ConstantAll.TYPE_ID_PHARMACIST);
...
//            record.setImtoken(ry.getToken());
    ...
    public PharmacistPo insertPharmacistSelective(Pharmacist record, String osType) {
        ...
        Pharmacy pharmacy = listAllPharmacy.stream().filter(t->t.getHospitalId().longValue() == record.getHospitalId().longValue()).findFirst().orElse(null);
        Assert.isTrue(pharmacy!=null, "您所选取的医院没有对应的药房配置，请配置药房!");
        record.setPharmacyId(pharmacy.getId());
        record.setPharmacyCodeNo(pharmacy.getPharmacyCodeNo());
        ...
        model.setToken(this.jwtTokenService.createToken(""+ConstantAll.TYPE_ID_PHARMACIST, model.getId().toString(), osType, model.getPhoneNumber()));
        model.setNonce(RandomUtils.randomNumeric(4));
        model.setAppKey(this.signatureService.getAppKeyBy(ConstantAll.TYPE_ID_PHARMACIST, model.getNonce()));
        ...
        doctorPracticePoint.setTypeId(ConstantAll.TYPE_ID_PHARMACIST);
    ...
    public String updateImtoken(Pharmacist record, boolean isNeedToken) {
        ...
        if (StringUtils.isBlank(record.getImtoken()) || isNeedToken) {
            RongYunTokenResult ry = this.serviceRongCloudService.getToken(ConstantAll.TYPE_ID_PHARMACIST, record.getId(), record.getTrueName(), record.getAvatarUrl());
            if (ry == null) {
                Assert.isTrue(false, "获取第三方即时通讯工具token错误");
            } else {
                Assert.isTrue(ry.getCode() == 200, ry.getMsg());
    ...
    public PharmacistPo loginByVerifyCode(PharmacistRequest params, String verifyCode, String osType) {
        ...
        smsService.checkCode(model.getPhoneNumber(), ConstantAll.TYPE_ID_PHARMACIST, verifyCode);
    ...
    public PharmacistPo insertPharmacistWithPic(PharmacistPo record, MultipartFile avatarPic, MultipartFile photoPic, MultipartFile idCardPic, Long userId) {
        ...
        auditInfo.setAuditType((int)ConstantAll.TYPE_ID_PHARMACIST);
    ...
    public PharmacistPo updatePharmacistWithPic(PharmacistPo record, MultipartFile avatarPic, MultipartFile photoPic, MultipartFile idCardPic, Long userId) {
        ...
        if(model.getPracticePoint() != null && !"".equals(model.getPracticePoint())){
            doctorPracticePoint.setId(Long.parseLong(model.getPracticePoint()));
